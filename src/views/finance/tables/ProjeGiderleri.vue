  ...
  <div class="fpage">
    <DataTable :value="db.orders" dataKey="id" v-model:expandedRows="expandedRows"
      ...
      <Column header="Toplam Gider" style="width: 170px">
        <template #body="{ data }"><span class="mono">{{ fmtMoney(totalExp(data), data.currency || 'EUR') }}</span></template>
      </Column>

      <template #expansion="{ data }">
        <div class="inst">
          <div class="inst-head">
            <span><i class="pi pi-arrow-up-right" /> {{ data.orderNo }} Giderleri <small>({{ data.currency || 'EUR' }})</small></span>
          ...
          <table class="inst-table">
            ...
            <tbody>
              <tr v-for="(e, i) in sortedExp(data)" :key="i">
                ...
                <td class="r mono">{{ fmtMoney(e.amount, data.currency || 'EUR') }}</td>
            ...
            <tfoot v-if="(data.expenses || []).length">
              <tr><td colspan="2">Toplam ({{ data.expenses.length }})</td><td class="r mono"><b>{{ fmtMoney(totalExp(data), data.currency || 'EUR') }}</b></td><td colspan="2"></td></tr>
            </tfoot>
          </table>
        </div>
      </template>
    ...
    <Dialog v-model:visible="dialog" :header="editTarget ? 'Gideri Düzenle' : 'Yeni Gider'" modal :style="{ width: '540px' }">
      <div class="form">
        <div class="field"><label>Açıklama *</label><InputText v-model="form.description" autofocus :invalid="submitted && !form.description" placeholder="Örn: Devreye alma – 1. etap" /></div>
        <div class="two">
...
import type { Order, OrderExpense } from "@/types";

const toast = useToast();
const expandedRows = ref<Order[]>([]);

const totalExp = (o: Order) => (o.expenses ?? []).reduce((s, e) => s + e.amount, 0);
const sortedExp = (o: Order) => [...(o.expenses ?? [])].sort((a, b) => (a.date < b.date ? -1 : 1));
const persist = (o: Order) => patchOrder(o.id, { expenses: o.expenses ?? [] });
...
function openEdit(o: Order, e: OrderExpense) {
  currentOrder.value = o;
...
function save() {
  ...
  if (editTarget.value) {
    ...
    o.expenses.push({ description: form.description, category: form.category || "Diğer", amount: form.amount, date });
    toast.add({ severity: "success", summary: "Gider eklendi", detail: form.description, life: 2000 });
  }
  persist(o);
  dialog.value = false;
}
function del(o: Order, e: OrderExpense) {
  const i = (o.expenses ?? []).indexOf(e);
  if (i >= 0) o.expenses!.splice(i, 1);
  persist(o);
}
</script>
