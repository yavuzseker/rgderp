  ...
  <div class="page">
    ...
    >
      <template #header>
        <PageHeader
          title="Tedarikçiler"
          subtitle="Üretim aşamalarını yürüten dış firma ve atölyeler"
      ...
      <Column field="name" header="Firma Adı" sortable>
        <template #body="{ data }">
          <div class="cell-name">
            <Avatar icon="pi pi-truck" shape="circle" style="background: #f59e0b1a; color: #f59e0b" />
            <span>{{ data.name }}</span>
          </div>
        </template>
      </Column>
    ...
    <Dialog v-model:visible="dialog" :header="form.id ? 'Tedarikçi Düzenle' : 'Yeni Tedarikçi'" modal :style="{ width: '460px' }">
      <div class="form">
        <div class="field">
          <label>Firma Adı *</label>
          <InputText v-model="form.name" autofocus :invalid="submitted && !form.name" placeholder="Örn: Demir CNC İmalat" />
          <small v-if="submitted && !form.name" class="err">Firma adı gerekli.</small>
        </div>
        <div class="field">
        ...
        <div class="field">
          ...
          <small class="hint">Tedarikçi bu token ile şifresiz giriş yapacak.</small>
        </div>
      </div>
      <template #footer>
...
import { db, saveSupplier, deleteSupplier, token } from "@/data/store";
...
const empty = (): Partial<Supplier> => ({ name: "", contact: "", accessToken: token() });
const form = reactive<Partial<Supplier>>(empty());
...
function confirmDelete(s: Supplier) {
  confirm.require({
    ...
    accept: () => {
      deleteSupplier(s.id);
      toast.add({ severity: "info", summary: "Silindi", detail: s.name, life: 2500 });
    },
  });
}
</script>
